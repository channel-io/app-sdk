...
- 28a8979: Add optional OAuth step presentation, semantic icons, result detail, and platform progress contracts. Existing hook actions remain unchanged. Requires platform support before apps opt in.

## 0.26.0

### Minor Changes

- 91c7950: Allow OAuth authorization and connection lifecycle hooks to declare an optional `authScope` of `channel` or `manager`. A matching scoped hook takes precedence over the shared hook of the same type; omitting the field preserves existing shared registration behavior. Reject duplicate OAuth hook type/scope registrations. Requires platform support for scoped OAuth hooks.
- c7467db: Add `sellerProductCode` to `CommerceProduct`, the code a seller assigns to a product by hand.
...
- ea88a07: Add Config action redirect results and per-action HTTPS redirect origins, with
  Proto-backed TypeScript schemas and Go types. Requires App Store support before
  applications enable redirects; existing action results remain compatible.
